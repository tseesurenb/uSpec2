'''
Created on June 12, 2025
Streamlined training procedure with MSE loss - No unnecessary pre-sampling
Optimized for spectral collaborative filtering models

@author: Tseesuren Batsuuri (tseesuren.batsuuri@hdr.mq.edu.au)
'''

import world
import numpy as np
import torch
import torch.nn as nn
import torch.nn.functional as F
import utils
from time import time
from tqdm import tqdm


class StreamlinedMSELoss:
    """Streamlined MSE Loss with on-the-fly negative sampling"""
    
    def __init__(self, model, config):
        self.model = model
        self.config = config
        base_lr = config['lr']
        weight_decay = config['decay']
        self.opt = torch.optim.Adam(model.parameters(), lr=base_lr, weight_decay=weight_decay)
        
        # MSE-specific parameters
        self.pos_weight = config.get('pos_weight', 1.0)
        self.neg_weight = config.get('neg_weight', 0.5)
        self.n_negatives = config.get('samples', 50)
    
    def sample_negatives_batch(self, users, pos_items, user_pos_sets, all_items):
        """Sample negatives on-the-fly for a batch"""
        batch_size = len(users)
        neg_items = np.zeros((batch_size, self.n_negatives), dtype=int)
        
        for i, user in enumerate(users):
            user_pos = user_pos_sets[user]
            user_negs = list(all_items - user_pos)
            
            if len(user_negs) >= self.n_negatives:
                neg_items[i] = np.random.choice(user_negs, size=self.n_negatives, replace=False)
            else:
                # If not enough negatives, sample with replacement
                neg_items[i] = np.random.choice(user_negs, size=self.n_negatives, replace=True)
        
        return neg_items
    
    def train_step(self, users, pos_items, user_pos_sets, all_items):
        """Train step with on-the-fly negative sampling"""
        self.opt.zero_grad()
        
        # Sample negatives for this batch
        neg_items = self.sample_negatives_batch(users, pos_items, user_pos_sets, all_items)
        
        # Convert to tensors
        batch_users = torch.tensor(users, dtype=torch.long, device=self.model.device)
        pos_items_tensor = torch.tensor(pos_items, dtype=torch.long, device=self.model.device)
        neg_items_tensor = torch.tensor(neg_items, dtype=torch.long, device=self.model.device)
        
        # Get full model predictions
        all_predictions = self.model(batch_users)
        
        # Positive samples loss
        pos_scores = all_predictions.gather(1, pos_items_tensor.unsqueeze(1)).squeeze()
        pos_targets = torch.ones_like(pos_scores)
        pos_loss = F.mse_loss(pos_scores, pos_targets, reduction='mean')
        
        # Negative samples loss
        neg_scores = all_predictions.gather(1, neg_items_tensor)  # [batch_size, n_neg]
        neg_targets = torch.zeros_like(neg_scores)
        neg_loss = F.mse_loss(neg_scores, neg_targets, reduction='mean')
        
        # Combine losses with weights
        reconstruction_loss = self.pos_weight * pos_loss + self.neg_weight * neg_loss
        
        # Regularization loss
        reg_loss = 0
        for param in self.model.parameters():
            if param.requires_grad:
                reg_loss += param.norm(2).pow(2)
        reg_loss = reg_loss * self.config['decay'] / (2 * len(users))
        
        # Total loss
        total_loss = reconstruction_loss + reg_loss
        
        total_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=1.0)
        self.opt.step()
        
        return pos_loss.cpu().item(), neg_loss.cpu().item(), total_loss.cpu().item()


def create_user_pos_sets(dataset):
    """Create user positive item sets for fast negative sampling"""
    user_pos_sets = {}
    for user_id in range(dataset.n_users):
        pos_items = dataset.allPos[user_id]
        user_pos_sets[user_id] = set(pos_items)
    return user_pos_sets


def train_epoch_mse_streamlined(dataset, model, loss_class, config):
    """Train one epoch with streamlined MSE loss - no pre-sampling needed"""
    model.train()
    
    # Prepare data structures for sampling
    train_users = dataset.trainUser
    train_items = dataset.trainItem
    user_pos_sets = create_user_pos_sets(dataset)
    all_items = set(range(dataset.m_items))
    
    # Shuffle training data for this epoch
    indices = np.random.permutation(len(train_users))
    shuffled_users = train_users[indices]
    shuffled_items = train_items[indices]
    
    train_batch_size = config['train_u_batch_size']
    n_batches = (len(train_users) + train_batch_size - 1) // train_batch_size
    
    total_pos_loss = 0.0
    total_neg_loss = 0.0
    total_loss = 0.0
    
    for batch_idx in range(n_batches):
        start_idx = batch_idx * train_batch_size
        end_idx = min(start_idx + train_batch_size, len(train_users))
        
        batch_users = shuffled_users[start_idx:end_idx]
        batch_pos_items = shuffled_items[start_idx:end_idx]
        
        # Train step with on-the-fly sampling
        pos_loss, neg_loss, batch_total_loss = loss_class.train_step(
            batch_users, batch_pos_items, user_pos_sets, all_items
        )
        
        total_pos_loss += pos_loss
        total_neg_loss += neg_loss
        total_loss += batch_total_loss
    
    return {
        'pos_loss': total_pos_loss / n_batches,
        'neg_loss': total_neg_loss / n_batches,
        'total_loss': total_loss / n_batches
    }


def evaluate_spectral(dataset, model, data_dict, config):
    """Evaluate spectral model with proper metrics"""
    if len(data_dict) == 0:
        return {'recall': np.zeros(len(world.topks)),
                'precision': np.zeros(len(world.topks)),
                'ndcg': np.zeros(len(world.topks))}
    
    model.eval()
    eval_batch_size = config['eval_u_batch_size']
    max_K = max(world.topks)
    
    results = {'recall': np.zeros(len(world.topks)),
               'precision': np.zeros(len(world.topks)),
               'ndcg': np.zeros(len(world.topks))}
    
    with torch.no_grad():
        users = list(data_dict.keys())
        all_results = []
        
        # Create interaction tensor for masking
        interactions_tensor = torch.zeros(dataset.n_users, dataset.m_items, device=world.device)
        for user_idx in range(dataset.n_users):
            pos_items = dataset.allPos[user_idx]
            if len(pos_items) > 0:
                interactions_tensor[user_idx, pos_items] = 1.0
        
        for batch_users in utils.minibatch(users, batch_size=eval_batch_size):
            batch_users = [int(u) for u in batch_users]
            
            ground_truth = [data_dict[u] for u in batch_users]
            
            batch_users_gpu = torch.LongTensor(batch_users).to(world.device)
            ratings = model.getUsersRating(batch_users_gpu)
            
            if isinstance(ratings, np.ndarray):
                ratings = torch.from_numpy(ratings).to(world.device)
            
            # Mask out training items
            ratings = ratings * (1 - interactions_tensor[batch_users_gpu])
            
            # Get top-K predictions
            _, top_items = torch.topk(ratings, k=max_K)
            
            # Compute metrics
            batch_result = compute_metrics(ground_truth, top_items.cpu().numpy())
            all_results.append(batch_result)
        
        # Aggregate results
        for result in all_results:
            results['recall'] += result['recall']
            results['precision'] += result['precision']
            results['ndcg'] += result['ndcg']
        
        # Average
        n_users = len(users)
        results['recall'] /= n_users
        results['precision'] /= n_users
        results['ndcg'] /= n_users
    
    return results


def compute_metrics(ground_truth, predictions):
    """Compute metrics (borrowed from utils)"""
    relevance = utils.getLabel(ground_truth, predictions)
    
    recall, precision, ndcg = [], [], []
    for k in world.topks:
        ret = utils.RecallPrecision_ATk(ground_truth, relevance, k)
        recall.append(ret['recall'])
        precision.append(ret['precision'])
        ndcg.append(utils.NDCGatK_r(ground_truth, relevance, k))
    
    return {'recall': np.array(recall),
            'precision': np.array(precision),
            'ndcg': np.array(ndcg)}


def train_and_evaluate_spectral(dataset, model, config):
    """Streamlined training pipeline with MSE loss - no pre-sampling overhead"""
    
    # Check validation
    has_validation = hasattr(dataset, 'valDict') and len(dataset.valDict) > 0
    
    # Initialize streamlined MSE loss
    loss_class = StreamlinedMSELoss(model, config)
    best_ndcg = 0.0
    best_epoch = 0
    best_model_state = None
    no_improvement = 0
    
    # Training parameters
    total_epochs = config['epochs']
    patience = config['patience']
    min_delta = config['min_delta']
    eval_every = config['n_epoch_eval']
    
    model = model.to(world.device)
    
    print("🚀 Starting streamlined MSE training (no pre-sampling overhead)...")
    
    # Training loop
    training_start = time()
    
    pbar = tqdm(range(total_epochs), desc="Training Spectral CF with Streamlined MSE")
    
    for epoch in pbar:
        # Train with on-the-fly sampling
        loss_dict = train_epoch_mse_streamlined(dataset, model, loss_class, config)
        
        # Update progress bar
        pbar.set_postfix({
            'Pos': f"{loss_dict['pos_loss']:.4f}",
            'Neg': f"{loss_dict['neg_loss']:.4f}",
            'Total': f"{loss_dict['total_loss']:.4f}"
        })
        
        # Evaluate
        if (epoch + 1) % eval_every == 0 or epoch == total_epochs - 1:
            eval_data = dataset.valDict if has_validation else dataset.testDict
            eval_name = "validation" if has_validation else "test"
            
            results = evaluate_spectral(dataset, model, eval_data, config)
            current_ndcg = results['ndcg'][0]
            
            # Check improvement
            if current_ndcg > best_ndcg + min_delta:
                best_ndcg = current_ndcg
                best_epoch = epoch + 1
                best_model_state = {k: v.cpu().clone() for k, v in model.state_dict().items()}
                no_improvement = 0
                
                print(f"\nEpoch {epoch+1}: New best {eval_name} NDCG = {current_ndcg:.6f}")
            else:
                no_improvement += 1
                print(f"\nEpoch {epoch+1}: {eval_name} NDCG = {current_ndcg:.6f} (best: {best_ndcg:.6f})")
            
            # Early stopping
            if no_improvement >= patience // eval_every:
                print(f"Early stopping at epoch {epoch+1}")
                break
    
    # Restore best model
    if best_model_state is not None:
        print(f"Restoring best model from epoch {best_epoch}")
        model.load_state_dict(best_model_state)
        model = model.to(world.device)
    
    training_time = time() - training_start
    
    # Final evaluation
    print("Final test evaluation...")
    final_results = evaluate_spectral(dataset, model, dataset.testDict, config)
    
    print(f"\n🎯 \033[96mFinal Results (Streamlined MSE):\033[0m ⏱️ {training_time:.2f}s | 🏆 Epoch {best_epoch}")
    print(f"📊 R@20: \033[94m{final_results['recall'][0]:.4f}\033[0m | P@20: \033[94m{final_results['precision'][0]:.4f}\033[0m | NDCG@20: \033[95m{final_results['ndcg'][0]:.4f}\033[0m")
    
    return model, final_results