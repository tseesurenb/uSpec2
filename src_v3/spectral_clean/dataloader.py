...
class ML100K(BasicDataset):
    ...
    def __init__(self, path="../../data/ml-100k"):
        ...
        print(f"Loaded {len(train_data)} training interactions and {len(test_data)} test interactions")
        ...
        print(f"Dataset: {self.n_user} users, {self.m_item} items")
        print(f"Train users: {len(self.trainUniqueUsers)}, Test users: {len(self.testUniqueUsers)}")
        
        self.Graph = None
        print(f"ML-100K Sparsity : {(len(self.trainUser) + len(self.testUser))/self.n_user/self.m_item}")
        ...
        print(f"UserItemNet nnz: {self.UserItemNet.nnz}")
        
        # pre-calculate
        self._allPos = self.getUserPosItems(list(range(self.n_user)))
        print(f"Average items per user: {np.mean([len(pos) for pos in self._allPos]):.2f}")
        ...
        print(f"Test dict has {len(self.__testDict)} users")

    def _load_coo_file(self, file_path):
        """Load COO format file with proper header handling"""
        data = []
            ...
            with open(file_path, 'r') as f:
                for line_num, line in enumerate(f):
                    ...
                    if line:
                        ...
                        if len(parts) >= 2:
                            # Skip header line
                            if line_num == 0 and (parts[0].lower() == 'user_id' or not parts[0].isdigit()):
                            ...
                            try:
                                ...
                                continue
            print(f"Successfully loaded {len(data)} interactions from {file_path}")
        except FileNotFoundError:
            print(f"File not found: {file_path}")
            print("Please ensure the ML-100K data files are in the correct path")
            raise
        return data
    ...
    def getSparseGraph(self):
        if self.Graph is None:
            try:
                ...
                print(f"Error creating sparse graph: {e}")
                raise
        return self.Graph
...
class Loader(BasicDataset):
    ...
    def __init__(self,config = world.config,path="../data/gowalla"):
        ...
        self.split = config.get('A_split', False)  # Default to no splitting
        self.folds = config.get('A_n_fold', 100)  # Default fold value
        self.mode_dict = {'train': 0, "test": 1}
        self.mode = self.mode_dict['train']
        self.n_user = 0
        self.m_item = 0
