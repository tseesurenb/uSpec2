...
class Loader(BasicDataset):
    ...
    def __init__(self, config=None, path=None):
        if config is None:
            ...
            config = world.config
        if path is None:
            path = f"../../data/{config.get('dataset', 'gowalla')}"
        ...
        dataset_name = config.get('dataset', 'gowalla')
        if dataset_name == 'ml-100k':
            # ML-100k uses different file format (u1.base/u1.test)
            original_train_data = self._load_ml100k_data(path + '/u1.base')
            test_data = self._load_ml100k_data(path + '/u1.test')
            # Convert to expected format
            self.testUser = [x[0] for x in test_data]
            self.testItem = [x[1] for x in test_data]
        else:
            # Other datasets use standard format
        ...
        # Handle full training vs train/val split
        if config.get('full_training', False):
            # Use all training data without validation split
            self.trainUser = np.array([x[0] for x in original_train_data])
            self.trainItem = np.array([x[1] for x in original_train_data])
            self.valUser = np.array([])
            self.valItem = np.array([])
            self._valDataSize = 0
            print("Using full training data (no validation split)")
        else:
            # Create train/val split
            self._create_train_val_split(original_train_data, val_ratio)
        
        # Create UserItemNet with training data
        self.UserItemNet = csr_matrix(
            (np.ones(len(self.trainUser)), (self.trainUser, self.trainItem)),
            shape=(self.n_user, self.m_item)
        )
        ...
        print(f"{config.get('dataset', 'dataset')} ready: Train={self.trainDataSize}, Val={self.valDataSize}, Test={len(self.testUser)}")
        print(f"Users={self.n_users}, Items={self.m_items}")
    ...
    def _load_ml100k_data(self, data_file):
        """Load ML-100k format data (user item rating timestamp)"""
        data = []
        max_user = 0
        max_item = 0
        ...
        with open(data_file) as f:
            for line in f:
                if len(line.strip()) > 0:
                    parts = line.strip().split('\t')
                    user = int(parts[0]) - 1  # Convert to 0-indexed
                    item = int(parts[1]) - 1  # Convert to 0-indexed
                    # Ignore rating and timestamp
        ...
        if not hasattr(self, 'n_user'):
            self.n_user = max_user + 1
            self.m_item = max_item + 1
        else:
            self.n_user = max(self.n_user, max_user + 1)
            self.m_item = max(self.m_item, max_item + 1)
